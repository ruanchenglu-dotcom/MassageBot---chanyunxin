...
test('Drag and drop should work even when there are standby bookings in the system', async ({ page }) => {
  test.setTimeout(60000); // Tăng timeout cho toàn bộ test
  ...
  // ----- Step 2: Add a Normal Booking at 22:00 -----
  await page.getByText('預約').first().click();
  ...
  // Wait up to 5s for either Next button or Suggestion
  try {
    await expect(nextBtn).toBeVisible({ timeout: 5000 });
    await nextBtn.click();
  } catch (e) {
    // If Next button is not visible, it means there is no availability at 08:00.
    // Click the first Smart Suggestion (usually a yellow box)
    const suggestionBtn = page.locator('.bg-yellow-50 button').first();
  ...
  // Wait a bit for the swap to process
  await page.waitForTimeout(1000);

  // We only assert that no JavaScript crash occurred.
  // The swap might succeed or show "無法換位" depending on the dense real data, both are valid!
  // The critical thing is it must NOT crash.
  const isCrashed = await page.evaluate(() => window.isCrashed || false);
