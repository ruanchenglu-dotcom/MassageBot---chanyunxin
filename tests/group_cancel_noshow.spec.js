...
test.describe('Group Booking Cancel and NoShow E2E Test', () => {
  test('should prompt for group cancel and call update-status API with applyGroup: false when ONLY THIS CUSTOMER is selected', async ({ page }) => {
    // 1. Mock APIs
    ...
    const swalText = page.locator('.swal2-html-container', { hasText: '請問要取消整組預約還是僅此客人' });
    ...
    expect(updateStatusRequests[0].status).toBe("CANCELLED");
    expect(updateStatusRequests[0].applyGroup).toBe(false);
  });
  
  test('should prompt for group noshow and call update-status API for ALL group members when WHOLE GROUP is selected', async ({ page }) => {
    // 1. Mock APIs
    await page.route('/api/check-auth', async (route) => {
      await route.fulfill({
        status: 200,
    ...
    // Control Center NoShow Button
    const noshowBtn = page.locator('button', { hasText: '爽約' }).first();
    ...
    const swalText = page.locator('.swal2-html-container', { hasText: '請問要將整組預約設為爽約還是僅此客人' });
    await expect(swalText).toBeVisible();

    // Click "爽約全體" (Confirm Button)
    const confirmBtn = page.locator('.swal2-confirm', { hasText: '爽約全體' });
    await expect(confirmBtn).toBeVisible();
    await confirmBtn.click();

    await page.waitForTimeout(1000); 

    // Verify the intercepted requests (should be 2 requests, one for each group member)
    expect(updateStatusRequests.length).toBe(2);
    
    // Sort by rowId to easily check
    updateStatusRequests.sort((a, b) => parseInt(a.rowId) - parseInt(b.rowId));
    
    expect(updateStatusRequests[0].rowId).toBe("101");
    expect(updateStatusRequests[0].status).toBe("NOSHOW");
    expect(updateStatusRequests[0].applyGroup).toBe(false);
    
    expect(updateStatusRequests[1].rowId).toBe("102");
    expect(updateStatusRequests[1].status).toBe("NOSHOW");
    expect(updateStatusRequests[1].applyGroup).toBe(false);
  });
});
