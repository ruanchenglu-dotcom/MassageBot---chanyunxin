...
const AbsenceCheckModal = ({ data, staffList, bookings, statusData, localShifts, onClose, onConfirm, viewDate }) => {
    ...
    const handleCheck = (e) => {
        ...
        if (type === 'LATE') {
            ...
            const normViewDate = viewDate ? viewDate.replace(/-/g, '/') : null;
            const viewDateCustomShift = (normViewDate && leavingStaff?.customShifts && leavingStaff.customShifts[normViewDate]) ? leavingStaff.customShifts[normViewDate] : null;
            const sStartStr = safeLocalShifts[staffId]?.start || viewDateCustomShift?.start || leavingStaff?.start || leavingStaff?.shiftStart || '00:00';
            ...
            if (leavingStaff) {
                 ...
                 const normViewDate = viewDate ? viewDate.replace(/-/g, '/') : null;
                 const viewDateCustomShift = (normViewDate && leavingStaff?.customShifts && leavingStaff.customShifts[normViewDate]) ? leavingStaff.customShifts[normViewDate] : null;
                 staffEndStr = safeLocalShifts[staffId]?.end || viewDateCustomShift?.end || leavingStaff.end || leavingStaff.shiftEnd || '24:00';
                 sStartStr = safeLocalShifts[staffId]?.start || viewDateCustomShift?.start || leavingStaff.start || leavingStaff.shiftStart || '00:00';
            }
        ...
        const activeBookings = bookings.filter(b => {
            ...
            if (status.includes('取消') || status.includes('cancel')) return false;
            return true;
        });
        ...
        for (const b of activeBookings) {
            ...
            if (bStart < absEnd && bEnd > absStart) {
                ...
                const cleanKeys = staffKeys.map(k => k ? window.normalizeStaffId(String(k)) : '');
                if (cleanKeys.includes(window.normalizeStaffId(staffId))) {
                    hasDesignated = true;
                    conflictBooking = b;
                    break;
                }
        ...
        for (let t = absStart; t < absEnd; t += 5) {
            ...
            for (const s of staffList) {
                ...
                const normViewDate = viewDate ? viewDate.replace(/-/g, '/') : null;
                const viewDateCustomShift = (normViewDate && s.customShifts && s.customShifts[normViewDate]) ? s.customShifts[normViewDate] : null;
                const sStartStr = safeLocalShifts[s.id]?.start || sStatus.lateStart || viewDateCustomShift?.start || s.start || s.shiftStart || '00:00';
                const sEndStr = safeLocalShifts[s.id]?.end || viewDateCustomShift?.end || s.end || s.shiftEnd || '24:00';
...
const CheckInBoard = ({ staffList, statusData, onClose, onUpdateStatus, bookings, salaryData, viewDate }) => {
    ...
    const [localShifts, setLocalShifts] = React.useState({});
            ...
            <div className="bg-white w-full max-w-[95vw] rounded-t-xl shadow-2xl modal-animate flex flex-col max-h-[90vh] overflow-hidden">
                ...
                <div className="overflow-y-auto flex-1 p-2 space-y-2 bg-white custom-scrollbar">
                    {sortedStaffList.map(s => {
                        ...
                        const internalIncome = staffIncomeMap[s.id] ? staffIncomeMap[s.id].income : 0;
                        ...
                        const normViewDate = viewDate ? viewDate.replace(/-/g, '/') : null;
                        const viewDateCustomShift = (normViewDate && s.customShifts && s.customShifts[normViewDate]) ? s.customShifts[normViewDate] : null;
                        const displayStart = localShifts[s.id]?.start || current.lateStart || viewDateCustomShift?.start || s.start || s.shiftStart || '';
                        const displayEnd = localShifts[s.id]?.end || viewDateCustomShift?.end || s.end || s.shiftEnd || '';
 ...
 * ============================================================================
 */
const AvailabilityCheckModal = ({ onClose }) => {
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
            <div className="bg-white p-8 rounded-xl flex flex-col items-center">
                <i className="fas fa-spinner fa-spin text-4xl text-indigo-600 mb-4"></i>
                <div className="text-lg font-bold text-gray-700">載入中...</div>
                <button onClick={onClose} className="mt-4 px-4 py-2 bg-gray-200 font-bold rounded">關閉</button>
            </div>
        </div>
    );
};
...
const BillingModal = ({ activeItem, relatedItems, onConfirm, onCancel }) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/90 z-[90] flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-5xl rounded-2xl shadow-2xl modal-animate overflow-hidden flex flex-col max-h-[95vh]">
                ...
                <div className="p-4 flex-1 overflow-y-auto custom-scrollbar flex flex-col lg:flex-row gap-6">
                    ...
                    {/* PAYMENT METHODS */}
                    <div className="w-full lg:w-[260px] bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-4 shrink-0 h-fit">
                    ...
                    {/* VOUCHER METHODS */}
                    <div className="w-full lg:w-[320px] bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-4 shrink-0 h-fit">
                        <div className="pt-1">
