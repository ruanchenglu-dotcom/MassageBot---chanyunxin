...
window.SmartScheduler = (function() {
    ...
    const getCandidateResources = (currentResId) => {
        ...
        if (rId.includes('MULTI') || rId.includes('多功能') || rId.includes('多')) {
            targetType = 'MULTI';
        } else if (rId.includes('BED') || rId.includes('床') || rId.includes('BODY') || rId.includes('身')) {
            targetType = 'BED';
        } else {
            targetType = 'CHAIR';
        }
        ...
        if (window.SYSTEM_CONFIG?.SCALE?.FLOORS) {
            let floors = window.SYSTEM_CONFIG.SCALE.FLOORS;
            // Nếu là BED hoặc CHAIR, ta có thể lấy cả MULTI làm fallback nếu muốn,
            // nhưng để an toàn khi smart repacking (đảo chỗ), ta ưu tiên trả đúng type hiện tại.
            let matchedFloor = floors.find(f => f.type === targetType);
            if (matchedFloor) {
                for (let i = 1; i <= matchedFloor.count; i++) {
                    resources.push(`${matchedFloor.prefix}${i}`);
                }
            }
        }
        return resources;
    ...
    const normalizeRes = (res) => {
        ...
        let match = s.match(/^(CHAIR|BED|MULTI|OPP-CHAIR|OPP-BED|OPP_CHAIR|OPP_BED)-?(\d+)-?(\d+)?$/);
        if (match) {
        ...
        match = s.match(/^(CHAIR|BED|MULTI|OPP-CHAIR|OPP-BED|OPP_CHAIR|OPP_BED)\s*(\d+)$/);
        if (match) {
            let type = match[1].replace('_', '-');
            let floor = '1';
            if (type === 'OPP-CHAIR') { type = 'CHAIR'; floor = '2'; }
        ...
        match = s.match(/^(床|腳|椅|對面床|對面腳|對面椅|多功能|多)-?(\d+)-?(\d+)?$/);
        if (match) {
            let type = 'CHAIR';
            let floor = match[3] ? match[2] : '1';
