...
(function () {
    ...
    const BookingListView = ({ bookings, onCancelBooking, onInlineUpdate }) => {
        ...
        // Lấy danh sách Dịch vụ và Nhân viên (nếu có sẵn trên window object)
        ...
        const statusOptions = ['已預約', 'Running', '✅ 完成', '❌ Cancelled'];
                                ...
                                if (!isEditingThisRow) {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="p-4 whitespace-nowrap"><span className={`px-3 py-1 rounded-full text-sm font-bold shadow-sm ${statusClass}`}>{b.status}</span></td>
                                            ...
                                            <td className="p-4 whitespace-nowrap text-center sticky right-0 bg-opacity-90 z-10 space-x-2" style={{ backgroundColor: 'inherit' }}>
                                                ...
                                                <button onClick={() => onCancelBooking(b.rowId, '❌ Cancelled')} className="text-red-500 hover:text-white hover:bg-red-500 border border-red-200 hover:border-red-500 px-3 py-1.5 rounded transition-all shadow-sm" title="取消預約 (Cancel)">
                                                    ❌
                                                </button>
                                            </td>
                                        </tr>
