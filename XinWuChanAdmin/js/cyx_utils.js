...
(function () {
    ...
    window.formatResourceLabel = (rid, includeEmoji = false) => {
        ...
        const c_f2 = config.CHAIR_PREFIX_F2 || '2樓腳';
        const m_f3 = config.MULTI_PREFIX_F3 || '3樓多功能';
        const b_f4 = config.BED_PREFIX_F4 || '4樓床';
        
        let res = window.normalizeResourceId(rid);
        let emoji = '';
        
        if (res.startsWith('CHAIR-2-')) {
            res = res.replace('CHAIR-2-', c_f2);
            emoji = '👣 ';
        } else if (res.startsWith('MULTI-3-')) {
            res = res.replace('MULTI-3-', m_f3);
            emoji = '✨ ';
        } else if (res.startsWith('BED-4-')) {
            res = res.replace('BED-4-', b_f4);
            emoji = '🛏️ ';
        }
