...
const TimelineView = ({ timelineData, onEditPhase, liveStatusData, staffList, statusData, onOpenControlCenter, branch = 'main' }) => {
    ...
    const handleScroll = (index) => (e) => {
        if (isSyncing.current && lastScrollSource.current !== index) return;
        
        isSyncing.current = true;
        lastScrollSource.current = index;
        ...
        // Debounce reset
        clearTimeout(isSyncing.current.timeout);
        isSyncing.current.timeout = setTimeout(() => {
    ...
    if (window.SYSTEM_CONFIG?.SCALE?.FLOORS) {
        ...
        window.SYSTEM_CONFIG.SCALE.FLOORS.forEach(f => {
            ...
            } else if (f.type === 'MULTI' || f.type === 'BED') {
                bedRows = bedRows.concat(rows);
            }
        });

        timelineGroups.push({ title: '腳', rows: chairRows });
        timelineGroups.push({ title: '床', rows: bedRows });
    }
                ...
                {timelineGroups.map((group, index) => (
                    <div key={index} className="flex-1 min-w-0 h-full">
                        {renderTimelineContainer(
                            group.title, 
                            group.rows, 
                            el => scrollRefs.current[index] = el, 
                            handleScroll(index)
                        )}
                    </div>
                ))}
