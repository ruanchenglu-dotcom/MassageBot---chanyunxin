 ...
 * - Hỗ trợ ghi dữ liệu độc lập vào 6 cột trạng thái (R->W).
 * * 3. GIỮ NGUYÊN TOÀN BỘ TÍNH NĂNG LINE BOT.
 * =================================================================================================
 */
...
async function syncData() {
    try {
        // [CẬP NHẬT 1]: Đọc mở rộng từ A đến W (thay vì A:Q) để lấy các cột Status
        ...
        if (rowsBooking && rowsBooking.length > 0) {
            for (let i = 1; i < rowsBooking.length; i++) {
                ...
                cachedBookings.push({
                    ...
                    // [CẬP NHẬT 1]: Lấy dữ liệu các cột trạng thái (R->W)
                    Status1: row[17] || '', 
                    Status2: row[18] || '', 
                    Status3: row[19] || '', 
                    Status4: row[20] || '', 
...
// [CẬP NHẬT 2]: Nâng cấp API này để ghi vào cột trạng thái riêng (Status1...6)
