...
function checkAvailability(dateStr, timeStr, serviceDuration, serviceType, specificStaffIds = null, pax = 1, requireFemale = false) {
    ...
    if (specificStaffIds) {
        ...
        for (const id of idsToCheck) {
            if (offStaffIds.includes(id)) return false; // Thợ nghỉ
            if (requireFemale && !isFemale(id)) return false; // Yêu cầu nữ nhưng chọn nam
        }
    }
    ...
    if (requireFemale && !specificStaffIds) {
        ...
        if ((availableFemales - workingStaffBusy) < pax) return false; // Ước lượng
    } else {
...
// --- HÀM TẠO NÚT THỢ (NÂNG CẤP: LỌC THỢ NGHỈ) ---
function createStaffBubbles(selectedDate, filterFemale = false, excludedIds = []) {
    let list = STAFF_LIST;
    const minguoDate = formatMinguoDate(selectedDate);
    
    // 1. Lọc ra những thợ đang NGHỈ ngày hôm đó
    const staffOffToday = cachedSchedule
        .filter(s => s.date === minguoDate)
        .map(s => s.staffId);
    
    list = list.filter(s => !staffOffToday.includes(s.id));

    // 2. Lọc Nữ (nếu cần)
    if (filterFemale) list = list.filter(s => s.gender === 'F');
    
    // 3. Lọc thợ đã chọn (nếu đi nhóm)
...
async function handleEvent(event) {
  ...
  if (text.startsWith('Admin:')) { const action = text.split(':')[1]; userState[userId] = { step: 'ADMIN_PICK_STAFF', action: action }; const bubbles = createStaffBubbles(new Date().toISOString().split('T')[0]).map(b => { const str = JSON.stringify(b).replace(/StaffSelect/g, 'StaffOp'); return JSON.parse(str); }); return client.replyMessage(event.replyToken, { type: 'flex', altText: 'Pick Staff', contents: { type: 'carousel', contents: bubbles } }); }
  ...
  if (text === 'Staff:Pick') { 
      ...
      // NÂNG CẤP: Truyền ngày vào createStaffBubbles để lọc thợ nghỉ
      const bubbles = createStaffBubbles(currentState.date, currentState.isOil, currentState.selectedStaff); 
  ...
  if (text.startsWith('StaffSelect:')) {
      ...
      if (currentState.selectedStaff.length < currentState.pax) {
          // NÂNG CẤP: Truyền ngày vào createStaffBubbles
          const bubbles = createStaffBubbles(currentState.date, currentState.isOil, currentState.selectedStaff);
          const currentGuestIndex = currentState.selectedStaff.length + 1;
          bubbles.forEach(b => { b.body.contents[0].text = `選第 ${currentGuestIndex} 位技師 (Guest ${currentGuestIndex})`; b.body.contents[0].color = "#E91E63"; });
...
app.listen(port, () => {
    console.log(`Bot v40.1 (FIXED OFF-STAFF FILTER) running on ${port}`);
});