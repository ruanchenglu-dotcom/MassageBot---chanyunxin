    ...
    for (let i = 0; i < list.length; i += chunkSize) {
        ...
        for (let j = 0; j < chunk.length; j += 3) {
            ...
            const rowButtons = rowItems.map(s => ({
                "type": "button", "style": "secondary", "color": (s.gender === 'F' || s.gender === '女') ? "#F48FB1" : "#90CAF9", "height": "sm", "margin": "xs", "flex": 1, "action": { "type": "message", "label": s.name, "text": `StaffSelect:${s.id}` }
            }));
...
async function handleEvent(event) {
  ...
  if (userState[userId] && userState[userId].step === 'PHONE') { 
      ...
      await client.replyMessage(event.replyToken, {
          ...
          contents: {
            "type": "bubble",
            "hero": { "type": "image", "url": "https://images.unsplash.com/photo-1600334089648-b0d9d302427f?q=80&w=1000", "size": "full", "aspectRatio": "20:13", "aspectMode": "cover", "action": { "type": "uri", "uri": "https://www.google.com/maps/search/?api=1&query=163+Zhonghe+Road" } }, // Link Maps chuẩn
            "body": {
            ...
            "footer": { "type": "box", "layout": "vertical", "spacing": "sm", "contents": [ { "type": "box", "layout": "horizontal", "spacing": "sm", "contents": [ { "type": "button", "style": "secondary", "action": { "type": "uri", "label": "📍 導航 (Maps)", "uri": "https://www.google.com/maps/search/?api=1&query=163+Zhonghe+Road" }, "height": "sm" }, { "type": "button", "style": "secondary", "action": { "type": "uri", "label": "📞 致電 (Call)", "uri": "tel:0282459868" }, "height": "sm" } ] }, { "type": "button", "style": "link", "action": { "type": "message", "label": "🔍 查看/取消預約", "text": "Action:MyBooking" }, "color": "#aaaaaa", "height": "sm" } ] }
          }
      });
...
app.listen(port, () => {
    console.log(`Bot v54.0 (SAFE MODE - HTTPS) running on ${port}`);
});