 ...
 * VERSION: V146 (FULL FIX: LINE BOT & STATUS SYNC R-W)
 * AUTHOR: AI ASSISTANT
 * DATE: 2025/12/28
 * * [CÁC TÍNH NĂNG CHÍNH]:
 * 1. LINE BOT: Đặt lịch, Xem menu, Admin quản lý, Gửi thông báo.
 * 2. SYNC DATA: Đồng bộ dữ liệu 2 chiều với Google Sheet (Cột A -> W).
 * 3. UPDATE API: Cập nhật trạng thái chi tiết cho từng thợ (Status1 -> Status6).
 * =================================================================================================
 */
...
// --- SYNC DATA (QUAN TRỌNG: ĐỌC ĐẾN CỘT W) ---
async function syncData() {
    try {
        const resBooking = await sheets.spreadsheets.values.get({ 
            spreadsheetId: SHEET_ID, 
            range: `${BOOKING_SHEET}!A:W` // Đọc hết cột trạng thái mới
        });
        ...
        if (rows && rows.length > 1) {
            ...
            for (let i = 1; i < rows.length; i++) {
                ...
                STAFF_LIST.push({ id: cleanName, name: cleanName, gender: gender, shiftStart: shiftStart, shiftEnd: shiftEnd });
...
// [API UPDATE DETAILS - HỖ TRỢ CỘT TRẠNG THÁI RIÊNG]
app.post('/api/update-booking-details', async (req, res) => {
    try {
        ...
        if (body.serviceName) {
            await sheets.spreadsheets.values.update({ spreadsheetId: SHEET_ID, range: `${BOOKING_SHEET}!D${rowId}`, valueInputOption: 'USER_ENTERED', requestBody: { values: [[body.serviceName]] } });
        }
        if (body.staffId && body.staffId !== '随機') {
            await sheets.spreadsheets.values.update({ spreadsheetId: SHEET_ID, range: `${BOOKING_SHEET}!I${rowId}`, valueInputOption: 'USER_ENTERED', requestBody: { values: [[body.staffId]] } });
        }
        ...
        for (const field of staffFields) {
            ...
            if (val) {
                await sheets.spreadsheets.values.update({ spreadsheetId: SHEET_ID, range: `${BOOKING_SHEET}!${field.col}${rowId}`, valueInputOption: 'USER_ENTERED', requestBody: { values: [[val]] } });
            }
        }
...
async function handleEvent(event) {
  ...
  if (text === 'Time:Suggest') { const s = userState[userId]; if (!s) return client.replyMessage(event.replyToken, { type: 'text', text: '⚠️ Session expired. Please restart.' }); let requireFemale = false, requireMale = false; if (s.pref === 'OIL') { requireFemale = true; } else if (s.pref === 'FEMALE') { requireFemale = true; } else if (s.pref === 'MALE') { requireMale = true; } const bestSlots = findBestSlots(s.date, s.service, s.pax, requireFemale, requireMale); if (bestSlots.length === 0) { return client.replyMessage(event.replyToken, { type: 'text', text: '😢 Xin lỗi, không tìm thấy khung giờ nào phù hợp.' }); } const bubbles = bestSlots.map(slot => ({ "type": "bubble", "size": "micro", "body": { "type": "box", "layout": "vertical", "paddingAll": "sm", "contents": [ { "type": "text", "text": slot.timeStr, "weight": "bold", "size": "xl", "color": "#1DB446", "align": "center" }, { "type": "text", "text": `👍 Điểm: ${slot.score}`, "size": "xxs", "color": "#aaaaaa", "align": "center" }, { "type": "button", "style": "secondary", "height": "sm", "action": { "type": "message", "label": "Chọn", "text": `Time:${slot.timeStr}` }, "margin": "sm" } ] } })); return client.replyMessage(event.replyToken, { type: 'flex', altText: 'Gợi ý giờ tốt nhất', contents: { "type": "carousel", "contents": bubbles } }); }
...
app.listen(port, () => {
    console.log(`Bot V145 (Status Columns Sync R->W) running on ${port}`);
});