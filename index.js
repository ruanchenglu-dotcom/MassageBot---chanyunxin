// ==============================================================================
// PHIÊN BẢN V73.0 - ULTIMATE BACKEND (FULL BOT LOGIC + SERVER SYNC)
// ==============================================================================
...
// Biến lưu trữ dữ liệu tạm thời (Cache)
let STAFF_LIST = []; 
let cachedBookings = []; 
let cachedSchedule = []; 
let userState = {}; // Lưu trạng thái hội thoại của từng khách hàng (State Machine)
let globalStaffStatus = {}; // Lưu trạng thái nhân viên (Ready/Busy) trên Server

// 2. BẢNG GIÁ & DỊCH VỤ (Giống Frontend)
const SERVICES = {
...
// Logic kiểm tra còn chỗ trống không (Quan trọng cho Bot)
function checkAvailability(dateStr, timeStr, serviceDuration, serviceType, specificStaffIds = null, pax = 1, requireFemale = false) {
    ...
    for (const booking of cachedBookings) {
        // Kiểm tra lịch nghỉ toàn cửa hàng
...
// Tạo danh sách nhân viên chọn (Có filter Nữ)
    ...
    try {
        ...
        if (rowsBooking && rowsBooking.length > 0) {
            for (let i = 1; i < rowsBooking.length; i++) {
                ...
                cachedBookings.push({ rowId: rowId, startTimeString: `${dateStr} ${timeStr}`, duration: duration, type: type, category: category, staffId: row[8] || '隨機', pax: pax, customerName: `${row[2]} (${row[6]})`, serviceName: serviceStr, status: status, lineId: row[9], isOil: isOil });
            }
        }
        ...
        if (rows && rows.length > 1) {
            ...
            for (let i = 1; i < rows.length; i++) {
                ...
                if (staffName && staffName.trim() !== '') {
                    ...
                    let gender = 'M'; if (staffGenderRaw && (staffGenderRaw.trim() === '女' || staffGenderRaw.trim().toUpperCase() === 'F')) gender = 'F';
                    const shiftStart = rows[i][2] || '10:00'; const shiftEnd = rows[i][3] || '02:00';
                    ...
                    if (headerDates.length > 4) {
                        for (let j = 4; j < rows[i].length; j++) {
                            const status = rows[i][j]; const rawDateStr = headerDates[j]; 
                            if (status && rawDateStr && status.trim() !== '') {
                                const formattedDate = formatDateDisplay(rawDateStr); if (formattedDate) cachedSchedule.push({ date: formattedDate, staffId: cleanName });
                            }
                        }
                    }
                }
...
app.get('/api/info', async (req, res) => { 
    await syncData(); 
    res.json({ staffList: STAFF_LIST, bookings: cachedBookings, schedule: cachedSchedule, staffStatus: globalStaffStatus, resources: { chairs: MAX_CHAIRS, beds: MAX_BEDS } }); 
});

app.post('/api/update-staff-status', (req, res) => {
    const { staffId, statusData } = req.body;
    if (staffId && statusData) {
        globalStaffStatus[staffId] = statusData;
        res.json({ success: true, staffStatus: globalStaffStatus });
...
app.post('/api/update-status', async (req, res) => { const { rowId, status } = req.body; try { await sheets.spreadsheets.values.update({ spreadsheetId: SHEET_ID, range: `${BOOKING_SHEET}!H${rowId}`, valueInputOption: 'USER_ENTERED', requestBody: { values: [[ status ]] } }); await syncData(); res.json({ success: true }); } catch (e) { console.error(e); } });
...
// ==============================================================================
// 5. LINE BOT HANDLE EVENT (FULL LOGIC)
// ==============================================================================
...
app.listen(port, () => { console.log(`Bot v73.0 (Full Logic) running on ${port}`); });