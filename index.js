...
// --- HÀM XỬ LÝ SỐ ĐIỆN THOẠI "SIÊU MẠNH" ---
function normalizePhoneNumber(phone) {
    if (!phone) return '';
    // 1. Chuyển số Full-width (０-９) sang Half-width (0-9)
    let normalized = phone.replace(/[\uff01-\uff5e]/g, function(ch) {
        return String.fromCharCode(ch.charCodeAt(0) - 0xfee0);
    });
    // 2. Xóa tất cả ký tự không phải số
...
async function handleEvent(event) {
  ...
  // CHỐT ĐƠN (ĐÃ THÊM VALIDATE SĐT MỚI)
  if (userState[userId] && userState[userId].step === 'PHONE') { 
      // 1. NORMALIZE SĐT
      const cleanSdt = normalizePhoneNumber(text); 
      // 2. CHECK ĐỘ DÀI (ĐÀI LOAN 09xx THƯỜNG LÀ 10 SỐ)
      if (!/^\d{7,15}$/.test(cleanSdt)) return client.replyMessage(event.replyToken, { type: 'text', text: '⚠️ 號碼格式錯誤 (Lỗi định dạng sđt). 請輸入正確手機號碼。' }); 
      ...
      await client.replyMessage(event.replyToken, { type: 'flex', altText: '預約成功', contents: { "type": "bubble", "hero": { "type": "image", "url": "https://images.unsplash.com/photo-1600334089648-b0d9d302427f?q=80&w=1000", "size": "full", "aspectRatio": "20:13", "aspectMode": "cover", "action": { "type": "uri", "uri": "https://maps.google.com" } }, "body": { "type": "box", "layout": "vertical", "contents": [ { "type": "text", "text": "✅ 預約成功 (Confirmed)", "weight": "bold", "size": "xl", "color": "#1DB446", "align": "center" }, { "type": "separator", "margin": "md" }, { "type": "text", "text": `${hoTen} 您好 (${paxDisplay})`, "weight": "bold", "align": "center", "margin": "md" }, { "type": "text", "text": serviceName, "size": "sm", "align": "center", "margin": "sm", "color": "#555555" }, { "type": "text", "text": currentState.isOil ? "⭐ 需要油推 (Dầu)" : "正常指壓", "size": "xs", "color": "#E91E63", "align": "center" }, { "type": "text", "text": `技師: ${staffDisplay}`, "weight": "bold", "color": "#0000ff", "align": "center", "margin": "sm" }, { "type": "text", "text": `${minguoDate} ${gio}`, "weight": "bold", "size": "xl", "color": "#1DB446", "align": "center", "margin": "sm" } ] }, "footer": { "type": "box", "layout": "vertical", "spacing": "sm", "contents": [ { "type": "text", "text": "總金額: $" + totalPrice, "size": "lg", "color": "#E63946", "weight": "bold", "align": "center" }, { "type": "button", "style": "secondary", "action": { "type": "message", "label": "🔍 查看 / 取消預約", "text": "Action:MyBooking" } } ] } } }); 
...
app.listen(port, () => {
    console.log(`Bot v45.1 (FIXED PHONE INPUT) running on ${port}`);
});