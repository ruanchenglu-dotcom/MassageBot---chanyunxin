...
const SERVICES = {
    'CB_190': { name: '👑 帝王套餐 (190分)', price: 2500, duration: 190, type: 'BED', category: 'COMBO' },
    'CB_130': { name: '💎 豪華套餐 (130分)', price: 1800, duration: 130, type: 'BED', category: 'COMBO' },
    'CB_100': { name: '🔥 招牌套餐 (100分)', price: 1300, duration: 100, type: 'BED', category: 'COMBO' },
    'CB_70':  { name: '⚡ 精選套餐 (70分)',  price: 1000, duration: 70,  type: 'BED', category: 'COMBO' },
...
function checkAvailability(dateStr, timeStr, serviceDuration, serviceType, specificStaffIds = null, pax = 1, requireFemale = false) {
    ...
    const availableStaff = STAFF_LIST.length - offStaffIds.length - workingStaffBusy;
    ...
    if (requireFemale && !specificStaffIds) {
        ...
        if (availableFemales < pax) return false; 
    } else {
...
async function handleEvent(event) {
  ...
  // SỬA LOGIC: CHÂN THÌ BỎ QUA OIL, BODY THÌ HỎI OIL
  if (text.startsWith('Svc:')) {
      ...
      // Nếu là Chân -> Nhảy thẳng sang hỏi Số người
  ...
  if (userState[userId] && userState[userId].step === 'PHONE') { 
      const sdt = text.trim(); 
      if (!/^\d{8,15}$/.test(sdt)) return client.replyMessage(event.replyToken, { type: 'text', text: '⚠️ 號碼格式錯誤 (Lỗi định dạng sđt). 請輸入數字 (Vui lòng nhập số).' }); 
      ...
      const totalPrice = pricePerPerson * currentState.pax;

      // E-TICKET (FLEX MESSAGE)
      await client.replyMessage(event.replyToken, {
          type: 'flex', altText: '✅ 預約成功', 
          contents: {
            "type": "bubble",
            "hero": {
              "type": "image", "url": "https://images.unsplash.com/photo-1600334089648-b0d9d302427f?q=80&w=1000", "size": "full", "aspectRatio": "20:13", "aspectMode": "cover",
              ...
              "contents": [
                ...
                { "type": "separator", "margin": "xl" },
                {
                  ...
                  "contents": [
                    ...
                    { "type": "box", "layout": "baseline", "contents": [ { "type": "text", "text": "人數 (Khách)", "color": "#aaaaaa", "size": "sm", "flex": 2 }, { "type": "text", "text": paxDisplay, "wrap": true, "color": "#666666", "size": "sm", "flex": 4 } ] },
                    ...
                    currentState.isOil ? { "type": "text", "text": "⭐ 包含油推 (Đẩy dầu)", "size": "xs", "color": "#E91E63", "align": "center", "margin": "md" } : { "type": "spacer", "size": "xs" }
                  ]
                },
                { "type": "separator", "margin": "xl" },
            ...
            "footer": {
              "type": "box", "layout": "vertical", "spacing": "sm", "contents": [
                { "type": "box", "layout": "horizontal", "spacing": "sm", "contents": [
                        { "type": "button", "style": "secondary", "action": { "type": "uri", "label": "📍 導航 (Maps)", "uri": "https://maps.google.com" }, "height": "sm" },
                        { "type": "button", "style": "secondary", "action": { "type": "uri", "label": "📞 致電 (Call)", "uri": "tel:+886912345678" }, "height": "sm" }
                    ]
                },
                { "type": "button", "style": "link", "action": { "type": "message", "label": "🔍 查看/取消預約", "text": "Action:MyBooking" }, "color": "#aaaaaa", "height": "sm" }
              ]
            }
          }
      });
...
app.listen(port, () => {
    console.log(`Bot v38.0 (Group Pick + Smart Flow) running on ${port}`);
});