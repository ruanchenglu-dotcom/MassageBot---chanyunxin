...
const SERVICES = {
    ...
    // TRẠNG THÁI HỆ THỐNG (ADMIN DÙNG)
...
// ==============================================================================
// 3. HÀM HỖ TRỢ (HELPERS - CHUẨN NĂM 2025)
// ==============================================================================
...
// Tạo danh sách 7 ngày tới (Dương lịch 2025)
...
// Format ngày sang YYYY/MM/DD (Dương lịch 2025)
function formatDate2025(dateInput) {
    if (!dateInput) return "";
    try {
        let str = dateInput.toString().trim();
        // Nếu có giờ thì cắt bỏ
...
// Lấy thời gian hiện tại chuẩn 2025 (YYYY/MM/DD HH:mm)
...
// Parse chuỗi ngày giờ thành đối tượng Date để so sánh
...
// ==============================================================================
// 4. ĐỒNG BỘ DỮ LIỆU (SYNC DATA) - ĐỌC ĐÚNG CỘT A->K
// ==============================================================================
...
async function syncData() {
    try {
        ...
        if (rowsBooking && rowsBooking.length > 0) {
            // Bỏ qua dòng tiêu đề, chạy từ i=1
            for (let i = 1; i < rowsBooking.length; i++) {
                ...
                for (const key in SERVICES) {
                    if (serviceStr.includes(SERVICES[key].name.split('(')[0])) { 
                        ...
                        split = SERVICES[key].split; // Lấy thông tin chia gói
                        break; 
                    }
                }
                ...
                cachedBookings.push({
                    ...
                    staffId: row[8] || '隨機', 
                    pax: pax,
                    customerName: `${row[2]} (${row[6]})`, // Tên (SĐT)
...
// ==============================================================================
// 5. LOGIC KIỂM TRA TRỐNG & TẠO BUBBLES
// ==============================================================================
function checkAvailability(dateStr, timeStr, serviceDuration, serviceType, specificStaffIds = null, pax = 1, requireFemale = false) {
    ...
    for (const booking of cachedBookings) {
        ...
        if (startRequest < endExisting && endRequest > startExisting) {
            ...
            // COMBO tính đơn giản là chiếm tài nguyên
...
// ==============================================================================
// 7. BOT HANDLE EVENT (QUAN TRỌNG: ĐẦY ĐỦ LOGIC)
// ==============================================================================
async function handleEvent(event) {
  ...
  if (text.startsWith('Cat:')) { 
      ...
      return client.replyMessage(event.replyToken, { type: 'flex', altText: '選擇方案', contents: { "type": "bubble", "body": { "type": "box", "layout": "vertical", "contents": [ { "type": "text", "text": "選擇方案 (Gói)", "weight": "bold", "size": "xl", "align": "center" }, { "type": "separator", "margin": "md" }, ...buttons ] } } }); 
  }
...
app.listen(port, () => {
    console.log(`Bot v88.0 (RESTORE FULL & FIX BUTTONS) running on ${port}`);
});