// ==============================================================================
// PHIÊN BẢN V120 - FULL FEATURES & FIX TIMEZONE/NIGHT SHIFT
// ==============================================================================
...
function isWithinShift(staff, requestTimeStr) {
    ...
    if (endMins < startMins) { // Ca qua đêm
...
async function syncData() {
    try {
        ...
        if (rowsBooking && rowsBooking.length > 0) {
            for (let i = 1; i < rowsBooking.length; i++) {
                ...
                cachedBookings.push({
                    ...
                    staffId: row[8] || '隨機', pax: parseInt(row[5]||1), customerName: `${row[2]}`, serviceName: row[3], status: status, lineId: row[9] 
                });
            }
        }
                    ...
                    if (headerDates.length > 4) {
                        for (let j = 4; j < rows[i].length; j++) {
                            if (rows[i][j] && headerDates[j]) {
                                cachedSchedule.push({ date: formatDateDisplay(headerDates[j]), staffId: cleanName });
                            }
                        }
                    }
                }
        ...
        if (tempStaffList.length === 0) {
            for(let i=1; i<=20; i++) tempStaffList.push({id:`${i}號`, name:`${i}號`, gender:'F', shiftStart:'10:00', shiftEnd:'02:00'});
        }
...
async function ghiVaoSheet(data) {
    try {
        ...
        const colD_Service = data.dichVu + (data.isOil ? " (油推+$200)" : "");
        const valuesToWrite = [[ colA_Date, colB_Time, data.hoTen, colD_Service, data.isOil?"Yes":"", data.pax, data.sdt, data.trangThai, data.nhanVien, data.userId, timeCreate ]];
...
function checkAvailability(dateStr, timeStr, serviceDuration, serviceType, specificStaffIds = null, pax = 1, requireFemale = false, requireMale = false) {
    ...
    // Yêu cầu: Đặt trước ít nhất 30 phút so với hiện tại nếu đặt giờ rưỡi, hoặc đơn giản là > hiện tại
    // Ở đây ta dùng logic: startRequest > now.
    ...
    for (const booking of cachedBookings) {
        if (booking.staffId === 'ALL_STAFF' && booking.startTimeString.includes(displayDate)) return false; 
...
function generateTimeBubbles(selectedDate, serviceCode, specificStaffIds = null, pax = 1, requireFemale = false, requireMale = false) {
    ...
    for (const h of availableSlots) {
        ...
        // FIX: Truyền giờ ảo (25:30) vào checkAvailability để xử lý qua đêm đúng
        const checkTimeStr = `${hourInt.toString().padStart(2, '0')}:${minuteStr}`;
        
        if (checkAvailability(selectedDate, checkTimeStr, service.duration, service.type, specificStaffIds, pax, requireFemale, requireMale)) { 
            validSlots.push(h); 
        }
    }
    ...
    return {
        "type": "bubble", "size": "mega", "body": {
            "type": "box", "layout": "vertical", "contents": [
                ...
                createRow("👑 帝王套餐 (腳+身)", 190, 2000), createRow("💎 豪華套餐 (腳+身)", 130, 1500), createRow("🔥 招牌套餐 (腳+身)", 100, 999), createRow("⚡ 精選套餐 (腳+身)", 70, 900),
                { "type": "text", "text": "👣 足底按摩 (Foot)", "weight": "bold", "size": "md", "color": "#111111", "margin": "lg" },
                createRow("足底按摩", 120, 1500), createRow("足底按摩", 90, 999), createRow("足底按摩", 70, 900), createRow("足底按摩", 40, 500),
                { "type": "text", "text": "🛏️ 身體指壓 (Body)", "weight": "bold", "size": "md", "color": "#111111", "margin": "lg" },
                createRow("全身指壓", 120, 1500), createRow("全身指壓", 90, 999), createRow("全身指壓", 70, 900), createRow("半身指壓", 35, 500),
...
app.get('/api/info', async (req, res) => { 
    await syncData(); 
    res.json({ staffList: STAFF_LIST, bookings: cachedBookings, schedule: cachedSchedule, resources: { chairs: MAX_CHAIRS, beds: MAX_BEDS }, resourceState: SERVER_RESOURCE_STATE, staffStatus: SERVER_STAFF_STATUS }); 
});
...
app.post('/api/admin-booking', async (req, res) => { await ghiVaoSheet({ ...req.body, userId: 'ADMIN_WEB', trangThai: '已預約' }); res.json({ success: true }); });
app.post('/api/update-status', async (req, res) => { await updateBookingStatus(req.body.rowId, req.body.status); res.json({ success: true }); });
app.post('/api/update-booking-details', async (req, res) => { await updateBookingDetails(req.body.rowId, req.body.staffId, req.body.serviceName); res.json({ success: true }); });
...
async function handleEvent(event) {
  ...
  // --- MENU CHÍNH ---
  if (text === 'Action:Booking' || text === 'Menu' || text === '預約') {
      userState[userId] = {}; 
      return client.replyMessage(event.replyToken, { type: 'flex', altText: 'Menu', contents: { "type": "bubble", "body": { "type": "box", "layout": "vertical", "contents": [ 
          { "type": "text", "text": "請選擇服務類別 (Service)", "weight": "bold", "size": "lg", "align": "center", "color": "#1DB446" }, 
          { "type": "separator", "margin": "md" }, 
          { "type": "button", "style": "primary", "color": "#A17DF5", "margin": "md", "action": { "type": "message", "label": "🔥 套餐 (Combo)", "text": "Cat:COMBO" } }, 
          { "type": "button", "style": "secondary", "margin": "sm", "action": { "type": "message", "label": "👣 足底按摩 (Foot)", "text": "Cat:FOOT" } }, 
          { "type": "button", "style": "secondary", "margin": "sm", "action": { "type": "message", "label": "🛏️ 身體指壓 (Body)", "text": "Cat:BODY" } } 
      ] } } });
  }

  // --- ADMIN BOT FEATURES (RESTORED) ---
  ...
  if (text.startsWith('Date:')) {
      ...
      userState[userId].date = selectedDate; 
      userState[userId].step = 'PREF';
      const serviceType = SERVICES[userState[userId].service].category;
      const buttons = [
      ...
      return client.replyMessage(event.replyToken, { type: 'flex', altText: 'Staff', contents: { "type": "bubble", "body": { "type": "box", "layout": "vertical", "contents": buttons } } });
  }

  if (text.startsWith('Pref:')) {
      userState[userId].pref = text.split(':')[1];
      ...
      return client.replyMessage(event.replyToken, { type: 'flex', altText: 'Pax', contents: { "type": "bubble", "body": { "type": "box", "layout": "vertical", "contents": [ { "type": "text", "text": "👥 請問幾位貴賓? (Pax)", "weight": "bold", "size": "lg", "align": "center", "color": "#1DB446" }, ...paxButtons ] } } });
  }

  if (text.startsWith('Pax:')) {
      const num = parseInt(text.split(':')[1]);
      userState[userId].pax = num;
      
      // RESTORED: Specific Staff Selection Flow
      if (userState[userId].pref === 'SPECIFIC') {
      ...
      if (userState[userId].pref === 'OIL') { userState[userId].isOil = true; requireFemale = true; }
      else if (userState[userId].pref === 'FEMALE') requireFemale = true;
      else if (userState[userId].pref === 'MALE') requireMale = true;

      const bubbles = generateTimeBubbles(userState[userId].date, userState[userId].service, null, num, requireFemale, requireMale);
      if(!bubbles) return client.replyMessage(event.replyToken, {type:'text',text:'😢 抱歉，該時段已客滿 (Full Booked)'});
      
      userState[userId].step = 'TIME';
      return client.replyMessage(event.replyToken, { type: 'flex', altText: 'Time', contents: bubbles });
  }

  // RESTORED: Handle Staff Selection for SPECIFIC
  if (text.startsWith('StaffSelect:')) {
      ...
      if (!userState[userId].selectedStaff) userState[userId].selectedStaff = [];
      userState[userId].selectedStaff.push(staffId);

      if (userState[userId].selectedStaff.length < userState[userId].pax) {
          const bubbles = createStaffBubbles(false, userState[userId].selectedStaff); 
          const nextIdx = userState[userId].selectedStaff.length + 1;
          bubbles.forEach(b => {
              b.body.contents[0].text = `選第 ${nextIdx}/${userState[userId].pax} 位技師`;
          ...
          const bubbles = generateTimeBubbles(userState[userId].date, userState[userId].service, userState[userId].selectedStaff, userState[userId].pax, false, false);
  ...
  if (userState[userId]?.step === 'PHONE') {
      ...
      let staffDisplay = s.pref==='RANDOM'?'隨機':s.pref;
      if (s.selectedStaff && s.selectedStaff.length > 0) staffDisplay = s.selectedStaff.join(', ');

      await ghiVaoSheet({ 
          ngayDen: s.date, gioDen: s.time, dichVu: SERVICES[s.service].name, 
          nhanVien: staffDisplay, userId: userId, 
          sdt: text, hoTen: s.name, trangThai: '已預約', pax: s.pax, isOil: s.isOil 
      });

      const confirmMsg = `✅ 預約成功\n\n👤 ${s.name} (${normalizePhoneNumber(text)})\n📅 ${s.date} ${s.time}\n💆 ${SERVICES[s.service].name}\n👥 ${s.pax} 位\n🛠️ ${staffDisplay}\n💵 總金額: $${totalPrice}`;
      delete userState[userId];
      return client.replyMessage(event.replyToken, { type: 'text', text: confirmMsg });
  }
...
app.listen(port, () => {
    console.log(`Bot V120 (Full Restore) running on ${port}`);
});