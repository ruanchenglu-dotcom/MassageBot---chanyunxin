...
// UPDATED PRICES V64.0
const SERVICES = {
  'CB_190': { name: '👑 帝王套餐 (190分)', duration: 190, type: 'BED', category: 'COMBO', price: 2000 },
  'CB_180': { name: '👑 帝王套餐 (180分)', duration: 180, type: 'BED', category: 'COMBO', price: 2000 }, // Added
  'CB_130': { name: '💎 豪華套餐 (130分)', duration: 130, type: 'BED', category: 'COMBO', price: 1500 },
  'CB_120': { name: '💎 豪華套餐 (120分)', duration: 120, type: 'BED', category: 'COMBO', price: 1500 }, // Added
  'CB_100': { name: '🔥 招牌套餐 (100分)', duration: 100, type: 'BED', category: 'COMBO', price: 999 },
  'CB_90':  { name: '🔥 招牌套餐 (90分)', duration: 90, type: 'BED', category: 'COMBO', price: 999 },   // Added
  ...
  'FT_35':  { name: '👣 足底按摩 (35分)', duration: 35, type: 'CHAIR', category: 'FOOT', price: 500 }, // Added
  ...
  'BD_40':  { name: '🛏️ 半身指壓 (40分)', duration: 40, type: 'BED', category: 'BODY', price: 500 }, // Added just in case
...
// ... [Keep Helper Functions: normalizePhoneNumber, getNext7Days, isFemale, isWithinShift, formatMinguoDate, getCurrentMinguoTime, parseMinguoToDate] ...
    ...
    try {
        ...
        for (let i = rows.length - 1; i >= 0; i--) {
            ...
            if (row[8] === userId && (!row[6] || !row[6].includes('取消'))) { 
                return { rowId: i + 1, thoiGian: `${row[0]} ${row[1]}`, dichVu: row[3], nhanVien: row[7], thongTinKhach: row[2] }; 
            } 
        }
        return null;
...
// ... [Check Availability & Bubbles - Same as before] ...
...
async function handleEvent(event) {
  ...
  if (userState[userId] && userState[userId].step === 'PHONE') { 
      ...
      // GỬI TIN NHẮN XÁC NHẬN (KHÔNG CÓ NÚT BẢN ĐỒ ĐỂ TRÁNH LỖI)
      await client.replyMessage(event.replyToken, { 
          type: 'flex', 
          altText: '✅ 預約成功', 
          contents: { 
            "type": "bubble", 
            "hero": { "type": "image", "url": "https://images.unsplash.com/photo-1600334089648-b0d9d302427f?q=80&w=1000", "size": "full", "aspectRatio": "20:13", "aspectMode": "cover" },
            "body": { "type": "box", "layout": "vertical", "contents": bodyContents },
            "footer": { "type": "box", "layout": "vertical", "spacing": "sm", "contents": [ { "type": "button", "style": "secondary", "action": { "type": "message", "label": "🔍 查看/取消預約 (My Booking)", "text": "Action:MyBooking" } } ] }
          } 
      });
...
app.listen(port, () => {
    console.log(`Bot v63.0 (CORRECT PRICE + NO MAPS) running on ${port}`);
});