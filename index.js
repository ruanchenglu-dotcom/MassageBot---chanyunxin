...
// -------------------------------------------------------------------------
// SERVER SETUP & ROUTING (QUAN TRỌNG: THỨ TỰ MIDDLEWARE ĐỂ LINE BOT HOẠT ĐỘNG)
// -------------------------------------------------------------------------
...
// [FIX QUAN TRỌNG]: Đặt Route Callback CỦA LINE LÊN TRƯỚC các body-parser khác
// Vì Line cần raw body để validate signature. Nếu để json() trước, nó sẽ nuốt mất raw body.
app.post('/callback', line.middleware(config), (req, res) => {
  Promise.all(req.body.events.map(handleEvent)).then((r) => res.json(r)).catch((e) => { console.error(e); res.status(500).end(); });
});

// Sau đó mới dùng body-parser cho các API khác (cho Kiosk/Admin)
...
// --- API UPDATE CHI TIẾT (CẬP NHẬT: GHI VÀO CỘT STATUS RIÊNG) ---
app.post('/api/update-booking-details', async (req, res) => {
    try {
        ...
        // --- CẬP NHẬT CÁC CỘT TRẠNG THÁI RIÊNG (R -> W) ---
        const statusFields = [
