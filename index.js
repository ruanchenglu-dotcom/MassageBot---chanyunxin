...
async function syncData() {
    try {
        // [CẬP NHẬT]: Đọc đến cột W để lấy dữ liệu Status1...Status6 (Cột R->W)
        const resBooking = await sheets.spreadsheets.values.get({ 
            spreadsheetId: SHEET_ID, 
            range: `${BOOKING_SHEET}!A:W` // Mở rộng range
        });
        ...
        if (rowsBooking && rowsBooking.length > 0) {
            for (let i = 1; i < rowsBooking.length; i++) {
                ...
                // Vẫn giữ lại đơn hoàn thành để hiện báo cáo
...
// -------------------------------------------------------------------------
// SERVER SETUP & ROUTING (ORDER FIXED)
// -------------------------------------------------------------------------
