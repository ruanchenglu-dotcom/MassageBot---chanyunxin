...
// Format ngày chuẩn 4 số: 0114/12/04
...
// ==============================================================================
// 2. DATA SYNC (ĐỌC ĐÚNG THỨ TỰ CỘT CŨ)
// ==============================================================================
    ...
    try {
        ...
        if (rowsBooking && rowsBooking.length > 0) {
            for (let i = 1; i < rowsBooking.length; i++) {
                ...
                // MAPPING CỘT THEO ẢNH 8FD227:
                // A[0]: Date, B[1]: Time, C[2]: Name, D[3]: Service, E[4]: Oil, F[5]: Pax, G[6]: Phone, H[7]: Status, I[8]: Staff, J[9]: ID, K[10]: Created
                ...
                cachedBookings.push({
                    ...
                    date: dateStr, 
                    time: timeStr, // Giữ nguyên giờ phút
                    duration: duration,
                    type: type,
...
async function ghiVaoSheet(data) {
    try {
        ...
        // --- GHI DỮ LIỆU ĐÚNG CỘT A->K CŨ ---
        // A:Date, B:Time, C:Name, D:Service, E:Oil, F:Pax, G:Phone, H:Status, I:Staff, J:LineID, K:CreatedTime

        let colA_Date = formatMinguoDisplay(data.ngayDen);     
        
        // FIX LỖI MẤT PHÚT: Giữ nguyên chuỗi giờ nếu có
        let colB_Time = data.gioDen || "";
        if (colB_Time.includes(' ')) colB_Time = colB_Time.split(' ')[1];
        if (colB_Time.length === 5) {
             // Giữ nguyên 10:00
        } else if (colB_Time.length > 5) {
             colB_Time = colB_Time.substring(0, 5);
        }
        ...
        await sheets.spreadsheets.values.append({
            ...
            range: 'Sheet1!A:A', // Tự động điền A->K
...
async function updateBookingStatus(rowId, newStatus) {
    try {
        // Trạng thái nằm ở Cột H (Index 7) -> Cột thứ 8
...
function generateTimeBubbles(selectedDate, serviceCode, specificStaffIds = null, pax = 1, requireFemale = false) {
    const now = new Date(); const taipeiNowStr = now.toLocaleString('en-US', { timeZone: 'Asia/Taipei', hour12: false }); const currentHour = parseInt(taipeiNowStr.split(', ')[1].split(':')[0]); const taipeiDate = new Date(taipeiNowStr); const todayStr = taipeiDate.toISOString().split('T')[0]; const isToday = (formatMinguoDisplay(selectedDate) === formatMinguoDisplay(todayStr));
...
app.post('/api/admin-staff-action', async (req, res) => { const { staffId, action, duration } = req.body; const now = new Date(); const taipeiNowStr = now.toLocaleString('en-US', { timeZone: 'Asia/Taipei', hour12: false }); const todayISO = formatMinguoDisplay(new Date(taipeiNowStr)); const currentTimeStr = taipeiNowStr.split(', ')[1].substring(0, 5); let serviceName = ''; let statusText = ''; if (action === 'break') { serviceName = `🍱 用餐 (${duration}m)`; statusText = '🍱 用餐中'; } else if (action === 'leave') { serviceName = `⛔ 早退 (${duration}m)`; statusText = '⚠️ 早退'; } await ghiVaoSheet({ gioDen: currentTimeStr, ngayDen: todayISO, dichVu: serviceName, nhanVien: staffId, userId: 'ADMIN_WEB', sdt: 'ADMIN', hoTen: '員工操作', trangThai: statusText, pax: 1, isOil: false }); res.json({ success: true }); });
...
app.listen(port, () => {
    console.log(`Bot v66.0 (Final Column Fix) running on ${port}`);
});