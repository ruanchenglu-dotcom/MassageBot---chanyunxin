...
// --- HÀM TẠO BẢNG GIÁ ĐẸP (FLEX MESSAGE) ---
...
async function handleEvent(event) {
  ...
  if (userState[userId] && userState[userId].step === 'PHONE') { 
      const sdt = normalizePhoneNumber(text); if (!/^\d{7,15}$/.test(sdt)) return client.replyMessage(event.replyToken, { type: 'text', text: '⚠️ 號碼格式錯誤。' }); 
...
app.listen(port, () => {
    console.log(`Bot v45.2 (PRICE UPDATE + MENU FIX) running on ${port}`);
});