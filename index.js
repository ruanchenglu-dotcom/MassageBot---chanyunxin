...
// ==============================================================================
// 1. CẤU HÌNH (ĐIỀN LẠI 4 THÔNG TIN CHÍNH XÁC)
// ==============================================================================
const config = {
...
// GIÁ TIỀN ĐÃ CẬP NHẬT CHUẨN
const SERVICES = {
    'CB_190': { name: '👑 帝王套餐 (190分)', duration: 190, type: 'BED', category: 'COMBO', price: 2000 },
    'CB_180': { name: '👑 帝王套餐 (180分)', duration: 180, type: 'BED', category: 'COMBO', price: 2000 },
    'CB_130': { name: '💎 豪華套餐 (130分)', duration: 130, type: 'BED', category: 'COMBO', price: 1500 },
    'CB_120': { name: '💎 豪華套餐 (120分)', duration: 120, type: 'BED', category: 'COMBO', price: 1500 },
    'CB_100': { name: '🔥 招牌套餐 (100分)', duration: 100, type: 'BED', category: 'COMBO', price: 999 },
    'CB_90':  { name: '🔥 招牌套餐 (90分)',  duration: 90,  type: 'BED', category: 'COMBO', price: 999 },
    ...
    'FT_35':  { name: '👣 足底按摩 (35分)',  duration: 35,  type: 'CHAIR', category: 'FOOT', price: 500 },
    ...
    'BD_40':  { name: '🛏️ 半身指壓 (40分)',  duration: 40,  type: 'BED', category: 'BODY', price: 500 },
...
function getNext7Days() { 
    ...
    for(let i=0; i<7; i++) { 
        ...
        if(i===0) l="今天 (Today)"; if(i===1) l="明天 (Tmrw)"; 
...
// --- SYNC DATA (THEO THỨ TỰ CỘT MỚI A->K) ---
// A:Date | B:Time | C:Name | D:Service | E:Oil | F:Pax | G:Phone | H:Status | I:Staff | J:UserID | K:Created
    ...
    try {
        ...
        if (rowsBooking && rowsBooking.length > 0) {
            for (let i = 1; i < rowsBooking.length; i++) {
                ...
                // Cột F (index 5) là Pax
                if (row[5]) pax = parseInt(row[5]);
                
                // Cột C (Name), G (Phone), I (Staff)
...
async function ghiVaoSheet(data) {
    try {
        ...
        await sheets.spreadsheets.values.append({
            spreadsheetId: SHEET_ID, range: `${BOOKING_SHEET}!A:K`, valueInputOption: 'USER_ENTERED',
            requestBody: { values: [[ 
                dateStr,        // A
                timeOnly,       // B
                data.hoTen,     // C
                finalService,   // D
                data.isOil ? 'Yes' : '', // E (Dầu)
                data.pax,       // F
                data.sdt,       // G
...
async function adminStaffAction(staffId, action, duration) {
    ...
    // Ghi vào Booking Sheet để hiện lên Timeline
    await sheets.spreadsheets.values.append({
        spreadsheetId: SHEET_ID, range: `${BOOKING_SHEET}!A:K`, valueInputOption: 'USER_ENTERED',
        requestBody: { values: [[ dateStr, currentTimeStr, 'Staff Action', serviceName, '', 1, '', statusText, staffId, 'ADMIN', currentTimeStr ]] }
    });
    await syncData();
}
...
async function layLichDatGanNhat(userId) {
    try {
        ...
        for (let i = rows.length - 1; i >= 0; i--) {
            const row = rows[i];
            // UserID cột J (index 9), Status cột H (index 7)
            if (row[9] === userId && (!row[7] || !row[7].includes('取消'))) { 
                return { rowId: i + 1, thoiGian: `${row[0]} ${row[1]}`, dichVu: row[3], nhanVien: row[8], thongTinKhach: row[2] }; 
            } 
        }
        return null;
...
// --- CHECK AVAILABILITY & GENERATE BUBBLES ---
...
async function handleEvent(event) {
  ...
  if (userState[userId] && userState[userId].step === 'PHONE') { 
      ...
      await client.replyMessage(event.replyToken, { type: 'flex', altText: '✅ 預約成功', contents: { "type": "bubble", "body": { "type": "box", "layout": "vertical", "contents": bodyContents }, "footer": { "type": "box", "layout": "vertical", "spacing": "sm", "contents": [ { "type": "button", "style": "primary", "action": { "type": "message", "label": "✅ 是的 (Book)", "text": "Action:Booking" } }, { "type": "button", "style": "secondary", "action": { "type": "message", "label": "📄 菜單 (Menu)", "text": "Menu" } } ] } } });
...
app.listen(port, () => {
    console.log(`Bot v62.1 (FINAL FIXED - SHEET COLUMNS) running on ${port}`);
});