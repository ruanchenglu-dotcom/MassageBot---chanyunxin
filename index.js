// ==============================================================================
// PHIÊN BẢN V74 - FULL FEATURES (SYNC + LOGIC CHI TIẾT)
// ==============================================================================
...
const SERVICES = {
    ...
    'LEAVE_EARLY': { name: '⚠️ 早退', duration: 0, type: 'NONE' }, // Duration tính động
    ...
    try {
        ...
        if (rowsBooking && rowsBooking.length > 0) {
            for (let i = 1; i < rowsBooking.length; i++) {
                ...
                // [Logic phục hồi từ V68]
                if (row[3] && row[3].includes('早退')) duration = parseInt(row[3].match(/\d+/)[0]);
                else {
        ...
        if (rows && rows.length > 1) {
            ...
            for (let i = 1; i < rows.length; i++) {
                ...
                if (staffName && staffName.trim() !== '') {
                    const cleanName = staffName.trim(); let gender = 'M'; if (rows[i][1] && (rows[i][1].trim() === '女' || rows[i][1].trim().toUpperCase() === 'F')) gender = 'F';
...
// --- LOGIC AVAILABILITY (FULL V68 + SYNC) ---
function checkAvailability(dateStr, timeStr, serviceDuration, serviceType, specificStaffIds = null, pax = 1, requireFemale = false) {
    ...
    // Check Bookings (bao gồm cả Admin Close Shop, Leave Early)
...
async function handleEvent(event) {
  ...
  if (userState[userId]?.step === 'PHONE') { 
      ...
      delete userState[userId]; return client.replyMessage(event.replyToken, { type: 'text', text: `✅ 預約成功!\n$${price}` });
  }
...
app.listen(port, () => console.log(`Bot V74 (Full Feature Sync) running on ${port}`));