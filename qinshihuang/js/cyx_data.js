...
const SYSTEM_CONFIG = {
    ...
    // Định dạng ID Tài nguyên Chuẩn & Phân bổ tầng
    RESOURCE_FORMAT: {
        MAIN: {
            CHAIR_PREFIX: 'CHAIR-2-',
            MULTI_PREFIX: 'MULTI-3-',
            BED_PREFIX: 'BED-4-'
        }
    },

    // Quy mô chi nhánh phân theo tầng
    SCALE: {
        FLOORS: [
            { level: 2, type: 'CHAIR', count: 16, prefix: 'CHAIR-2-' },
            { level: 3, type: 'MULTI', count: 6, prefix: 'MULTI-3-' },
            { level: 4, type: 'BED', count: 10, prefix: 'BED-4-' }
        ],
        // Giữ lại các biến cũ để tương thích ngược tạm thời nếu cần
        get MAX_CHAIRS() { return this.FLOORS.find(f => f.type === 'CHAIR').count; },
        get MAX_BEDS() { return this.FLOORS.find(f => f.type === 'BED').count + this.FLOORS.find(f => f.type === 'MULTI').count; },
        get TOTAL_RESOURCES() { return this.FLOORS.reduce((sum, floor) => sum + floor.count, 0); }
    },
...
const SERVICES_DATA = {
    ...
    'A3': { name: '套餐 (100分)', duration: 100, price: 999, type: 'BED', category: 'COMBO', blocks: 3, commission: 250, elasticStep: 1, minFoot: 30, maxFoot: 60, minBody: 40, maxBody: 70 },
    'A4': { name: '套餐 (130分)', duration: 130, price: 1500, type: 'BED', category: 'COMBO', blocks: 4, commission: 250, elasticStep: 1, minFoot: 30, maxFoot: 90, minBody: 40, maxBody: 100 },
    'A6': { name: '套餐 (190分)', duration: 190, price: 2200, type: 'BED', category: 'COMBO', blocks: 6, commission: 250, elasticStep: 1, minFoot: 40, maxFoot: 110, minBody: 80, maxBody: 150 },

    'A2': { name: '套餐 (70分)', duration: 70, price: 900, type: 'BED', category: 'COMBO', blocks: 2, commission: 250, elasticStep: 1, minFoot: 30, maxFoot: 40, minBody: 30, maxBody: 40 },
