...
(function () {
  ...
  const StaffSorter = {
    ...
    getRequestedStaffForGroupIndex: (booking, grpIdx) => {
      ...
      let targetStaff = '隨機';
      if (grpIdx === 0) targetStaff = booking.serviceStaff || booking.staffId || booking.ServiceStaff || booking.requestedStaff || '隨機';else if (grpIdx === 1) targetStaff = booking.staffId2 || booking.serviceStaff || booking.staffId || booking.requestedStaff || '隨機';else if (grpIdx === 2) targetStaff = booking.staffId3 || booking.serviceStaff || booking.staffId || booking.requestedStaff || '隨機';else if (grpIdx === 3) targetStaff = booking.staffId4 || booking.serviceStaff || booking.staffId || booking.requestedStaff || '隨機';else if (grpIdx === 4) targetStaff = booking.staffId5 || booking.serviceStaff || booking.staffId || booking.requestedStaff || '隨機';else if (grpIdx === 5) targetStaff = booking.staffId6 || booking.serviceStaff || booking.staffId || booking.requestedStaff || '隨機';
      return targetStaff;
    },
    ...
    getStaffIdFromPaymentItem: item => {
      ...
      let targetStaff = "";
      if (num === 1) targetStaff = b.serviceStaff || b.staffId || b.technician;else if (num === 2) targetStaff = b.staffId2;else if (num === 3) targetStaff = b.staffId3;else if (num === 4) targetStaff = b.staffId4;else if (num === 5) targetStaff = b.staffId5;else if (num === 6) targetStaff = b.staffId6;
    ...
    sortPaymentItems: (itemsToPay, statusData) => {
      ...
      sorted.sort((a, b) => {
        ...
        const priceA = (window.getPrice ? window.getPrice(bookingA.serviceName) : 0) + (window.getOilPrice ? window.getOilPrice(bookingA.isYouTui) : 0);
        const priceB = (window.getPrice ? window.getPrice(bookingB.serviceName) : 0) + (window.getOilPrice ? window.getOilPrice(bookingB.isYouTui) : 0);
    ...
    organizeStaff: (staffList, statusData, resourceState) => {
      ...
      safeStaffList.forEach(s => {
        const currentStat = safeStatus[s.id] || {
          status: 'AWAY'
        };
    ...
    checkFutureAvailability: (staffId, proposedDuration, allBookings, currentMins, currentRowId, currentPhone) => {
      ...
      for (const b of allBookings) {
        ...
        const keys = [b.serviceStaff, b.staffId, b.ServiceStaff, b.technician, b.staffId2, b.staffId3, b.staffId4, b.staffId5, b.staffId6];
      ...
      return staffList.map(staff => {
        ...
        if (todaysBookings && Array.isArray(todaysBookings)) {
          todaysBookings.forEach(b => {
            ...
            const assignedStaffs = [b.technician, b.staffId, b.requestedStaff];
            const isDesignated = assignedStaffs.some(req => {
              if (!req) return false;
              const reqStr = String(req).trim();
              return reqStr !== '隨機' && reqStr !== '男' && reqStr !== '女' && reqStr !== '男師' && reqStr !== '女師' && reqStr !== 'MALE' && reqStr !== 'FEMALE' && StaffSorter.normalizeStaffId(reqStr) === StaffSorter.normalizeStaffId(staff.id);
            });
            if (isDesignated) {
        ...
        return {
          ...staff,
          hasUpcomingDesignated: hasUpcoming,
          upcomingDesignatedBookings
        };
      });
    },
    // =========================================================================
    ...
    processCheckIn: (staffId, statusData, staffList, newCheckInTime) => {
      ...
      if (staffList && Array.isArray(staffList)) {
        ...
        const readyTimes = Object.entries(statusData || {}).filter(([id, staff]) => {
          const sStatus = staff.status;
          return sStatus === 'READY' || sStatus === 'EAT' || sStatus === 'OUT_SHORT';
        }).map(([_, staff]) => Number(staff.stafftime)).filter(t => !isNaN(t));
    ...
    processCheckout: async (checkoutStaffInfo, currentStatusData, staffList, baseTime = Date.now(), isGroup = false) => {
      ...
      checkoutStaffInfo.forEach(info => {
        ...
        const currentStaffTime = currentStatusData[info.staffId]?.previousStafftime || currentStatusData[info.staffId]?.stafftime || baseTime;
