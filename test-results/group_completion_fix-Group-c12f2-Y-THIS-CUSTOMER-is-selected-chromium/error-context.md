...
- Name: group_completion_fix.spec.js >> Group Booking Completion E2E Test >> should prompt for group update and call update-status API with applyGroup: false when ONLY THIS CUSTOMER is selected
- Location: tests\group_completion_fix.spec.js:4:3
...
> 107 |     await expect(bookingBlock).toBeVisible({ timeout: 10000 });
  ...
  110 |     
  111 |     // Control Center Finish Button
  112 |     const finishBtn = page.locator('button', { hasText: '完' }).first();
  113 |     await expect(finishBtn).toBeVisible({ timeout: 5000 });
  114 |     await finishBtn.click();
  115 | 
  ...
  119 |     
  120 |     const swalText = page.locator('.swal2-html-container', { hasText: '此為團體客' });
  ...
  133 |     expect(updateStatusRequests[0].status).toBe("已完成");
