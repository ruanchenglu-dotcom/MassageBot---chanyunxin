...
const staffList = {
    'S1': { id: 'S1', name: 'S1', status: 'WORKING', schedule: '08:00-22:00' },
    'S2': { id: 'S2', name: 'S2', status: 'WORKING', schedule: '08:00-22:00' }
};

for (let h = 17; h <= 18; h++) {
    for (let m = 0; m < 60; m++) {
        let tStr = `${h}:${m.toString().padStart(2, '0')}`;
        let res = checkRequestAvailability('2026-07-06', tStr, guests, bookings, staffList, { location: '本館' });
        if (res.feasible) {
            console.log(`[SUCCESS] ${tStr} fits! -> ${res.details[0].phase1_duration}/${res.details[0].phase2_duration}`);
        }
    }
}
