// ==============================================================================
// SERVER V87 - FINAL MERGE (Full LINE Bot V56 + Server State V86)
// ==============================================================================
...
const DB_FILE = 'sessions.json'; // File lưu trạng thái
const MAX_CHAIRS = 6; 
const MAX_BEDS = 6;   
...
function readSessionDB() {
    try {
        if (!fs.existsSync(DB_FILE)) {
            const initialData = { sessions: {}, completedIds: [] };
            fs.writeFileSync(DB_FILE, JSON.stringify(initialData), 'utf8');
            return initialData;
        }
        const data = fs.readFileSync(DB_FILE, 'utf8');
        return JSON.parse(data || '{"sessions":{}, "completedIds":[]}');
    } catch (e) {
        console.error("Lỗi đọc DB:", e);
        return { sessions: {}, completedIds: [] };
    }
}
...
// HELPERS
function normalizePhoneNumber(phone) { return phone ? phone.replace(/[^0-9]/g, '') : ''; }
function isWithinShift(staff, requestTimeStr) {
    ...
    return endMins > startMins ? (requestMins >= startMins && requestMins < endMins) : (requestMins >= startMins || requestMins < endMins);
}
...
function parseStringToDate(dateStr) {
    ...
    try {
        const parts = dateStr.trim().split(' '); const dateNums = parts[0].split('/'); const timeNums = (parts[1]||"00:00").split(':');
    ...
    try {
        ...
        if (rowsBooking && rowsBooking.length > 0) {
            for (let i = 1; i < rowsBooking.length; i++) {
                ...
                for (const key in SERVICES) { if ((row[3]||'').includes(SERVICES[key].name.split('(')[0])) { duration = SERVICES[key].duration; type = SERVICES[key].type; break; } }
                cachedBookings.push({
                    rowId: i + 1,
                    startTimeString: `${row[0]} ${row[1]}`,
                    date: row[0], time: row[1],
                    duration: duration, type: type, staffId: row[8] || '隨機', pax: parseInt(row[5]||1),
        ...
        if (rows && rows.length > 1) {
            ...
            for (let i = 1; i < rows.length; i++) {
                const cleanName = rows[i][0].trim();
                if (cleanName) {
                    let gender = 'M'; if ((rows[i][1]||'').toUpperCase().includes('F') || rows[i][1] === '女') gender = 'F';
            ...
            if (tempStaffList.length > 0) STAFF_LIST = tempStaffList; else for(let i=1; i<=20; i++) STAFF_LIST.push({id:`${i}號`, name:`${i}號`, gender:'F', shiftStart:'00:00', shiftEnd:'24:00'});
...
async function ghiVaoSheet(data) {
    try {
        ...
        let colA = formatDateDisplay(data.ngayDen); let colB = (data.gioDen||"").split(' ')[1] || data.gioDen; if (colB.length > 5) colB = colB.substring(0, 5);
        let colD = data.dichVu + (data.isOil ? " (油推+$200)" : "");
        await sheets.spreadsheets.values.append({ spreadsheetId: SHEET_ID, range: 'Sheet1!A:A', valueInputOption: 'USER_ENTERED', requestBody: { values: [[ colA, colB, data.hoTen||'現場客', colD, data.isOil?"Yes":"", data.pax||1, data.sdt, data.trangThai||'已預約', data.nhanVien||'隨機', data.userId, timeCreate ]] } });
        await syncData();
    } catch (e) { console.error('[ERROR] Lỗi ghi Sheet:', e); }
}

async function updateBookingStatus(rowId, newStatus) {
    try { await sheets.spreadsheets.values.update({ spreadsheetId: SHEET_ID, range: `${BOOKING_SHEET}!H${rowId}`, valueInputOption: 'USER_ENTERED', requestBody: { values: [[ newStatus ]] } }); await syncData(); } catch (e) { console.error('Update Error:', e); }
}

async function layLichDatGanNhat(userId) {
    try {
        const res = await sheets.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: `${BOOKING_SHEET}!A:K` }); const rows = res.data.values; if (!rows) return null;
        for (let i = rows.length - 1; i >= 0; i--) { const row = rows[i]; if (row[9] === userId && !row[7].includes('取消') && !row[7].includes('Cancelled')) return { rowId: i + 1, thoiGian: `${row[0]} ${row[1]}`, dichVu: row[3], nhanVien: row[8], thongTinKhach: `${row[2]} (${row[6]})`, chiTiet: row }; }
        return null;
    } catch (e) { return null; }
}

// 3. LOGIC (Full Bot Logic from V56)
function checkAvailability(dateStr, timeStr, serviceDuration, serviceType, specificStaffIds = null, pax = 1, requireFemale = false) {
    ...
    const startRequest = parseStringToDate(`${displayDate} ${timeStr}`); if (!startRequest) return false;
    ...
    const workingStaffs = STAFF_LIST.filter(staff => {
        ...
        return isWithinShift(staff, timeStr);
    });

    if (specificStaffIds) { const ids = Array.isArray(specificStaffIds)?specificStaffIds:[specificStaffIds]; for(const id of ids) if(!workingStaffs.some(s=>s.id===id)) return false; }
    ...
    for (const booking of cachedBookings) {
        ...
        const startExisting = parseStringToDate(booking.startTimeString); if (!startExisting) continue;
        ...
        if (startRequest < endExisting && endRequest > startExisting) {
            ...
            if (specificStaffIds) { const bookedStaffs = booking.staffId.split(',').map(s=>s.trim()); const ids = Array.isArray(specificStaffIds)?specificStaffIds:[specificStaffIds]; for(const reqId of ids) if(bookedStaffs.includes(reqId)) isSpecificStaffBusy = true; }
        }
    }
...
function generateTimeBubbles(selectedDate, serviceCode, specificStaffIds = null, pax = 1, requireFemale = false) {
    ...
    const bubbles = groups.filter(g => g.slots.length > 0).map(group => {
        const buttons = group.slots.map(h => {
            const timeStr = formatTime(h); const valueToSend = h < 24 ? `${h.toString().padStart(2, '0')}:00` : `${(h - 24).toString().padStart(2, '0')}:00`;
...
app.post('/callback', line.middleware(config), (req, res) => { Promise.all(req.body.events.map(handleEvent)).then((r) => res.json(r)).catch((e) => res.status(500).end()); });

// API INFO (SERVER STATE)
app.get('/api/info', async (req, res) => { 
    await syncData(); 
    const dbData = readSessionDB(); 
    res.json({ staffList: STAFF_LIST, bookings: cachedBookings, schedule: cachedSchedule, resources: { chairs: MAX_CHAIRS, beds: MAX_BEDS }, resourceState: dbData.sessions || {}, completedIds: dbData.completedIds || [] }); 
});

app.post('/api/update-state', (req, res) => { writeSessionDB(req.body); res.json({ success: true }); });
...
app.post('/api/admin-booking', async (req, res) => { await ghiVaoSheet({ ngayDen: req.body.ngayDen, gioDen: req.body.gioDen, dichVu: req.body.dichVu, nhanVien: req.body.nhanVien, userId: 'ADMIN_WEB', sdt: req.body.sdt||'現場客', hoTen: req.body.hoTen||'現場客', trangThai: '已預約', pax: req.body.pax||1, isOil: req.body.isOil }); res.json({ success: true }); });
app.post('/api/admin-staff-action', async (req, res) => { 
    const { staffId, action, duration } = req.body; const now = new Date(); const taipeiNowStr = now.toLocaleString('en-US', { timeZone: 'Asia/Taipei', hour12: false }); const todayISO = formatDateDisplay(new Date(taipeiNowStr)); const currentTimeStr = taipeiNowStr.split(', ')[1].substring(0, 5); let serviceName = ''; let statusText = ''; 
    if (action === 'break') { serviceName = `🍱 用餐 (${duration}分)`; statusText = '🍱 用餐中'; } else if (action === 'leave') { serviceName = `⛔ 早退 (${duration}分)`; statusText = '⚠️ 早退'; } 
    await ghiVaoSheet({ gioDen: currentTimeStr, ngayDen: todayISO, dichVu: serviceName, nhanVien: staffId, userId: 'ADMIN_WEB', sdt: 'ADMIN', hoTen: '員工操作', trangThai: statusText, pax: 1, isOil: false }); res.json({ success: true }); 
});

// 5. BOT HANDLE EVENT (RESTORED)
async function handleEvent(event) {
  ...
  if (text.startsWith('DatePick:') && userState[userId]?.step === 'ADMIN_PICK_CLOSE_DATE') { const pickedDate = text.split(':')[1]; await ghiVaoSheet({ gioDen: '08:00', ngayDen: pickedDate, dichVu: SERVICES['SHOP_CLOSE'].name, nhanVien: 'ALL_STAFF', userId: 'ADMIN', sdt: 'ADMIN', hoTen: '全店店休', trangThai: '⛔ 店休' }); delete userState[userId]; return client.replyMessage(event.replyToken, { type: 'text', text: `✅ 已設定 ${pickedDate} 全店店休。` }); }
  ...
  if (text.startsWith('StaffOp:')) { 
      ...
      const now = new Date(); const taipeiNowStr = now.toLocaleString('en-US', { timeZone: 'Asia/Taipei', hour12: false }); const todayISO = formatDateDisplay(new Date(taipeiNowStr)); const currentTimeStr = taipeiNowStr.split(', ')[1].substring(0, 5); let logType = ''; let logNote = ''; 
      ...
      else if (currentState.action === 'SetLeaveEarly') { logType = '早退/病假'; let effectiveHour = new Date(taipeiNowStr).getHours(); if (effectiveHour < 8) effectiveHour += 24; const currentTotalMins = effectiveHour * 60 + new Date(taipeiNowStr).getMinutes(); let duration = (26 * 60) - currentTotalMins; if (duration < 0) duration = 0; logNote = `早退 (${duration}分)`; await ghiVaoSheet({ gioDen: currentTimeStr, ngayDen: todayISO, dichVu: `⛔ 早退 (${duration}分)`, nhanVien: staffId, userId: 'ADMIN', sdt: 'ADMIN', hoTen: '管理員操作', trangThai: '⚠️ 早退' }); } 
  ...
  if (text.startsWith('Date:')) { const selectedDate = text.split(':')[1]; const currentState = userState[userId]; if (!currentState) return client.replyMessage(event.replyToken, { type: 'text', text: '請重新預約。' }); currentState.date = selectedDate; userState[userId] = currentState; return client.replyMessage(event.replyToken, { type: 'template', altText: '選師傅', template: { type: 'buttons', text: `共有 ${currentState.pax} 位貴賓。請問是否指定師傅？`, actions: [ { type: 'message', label: '不指定 (隨機)', text: 'Staff:Random' }, { type: 'message', label: '指定師傅', text: 'Staff:Pick' } ] } }); }
  ...
  if (text.startsWith('Time:')) { const gio = text.replace('Time:', '').trim(); const currentState = userState[userId]; if (!currentState) return client.replyMessage(event.replyToken, { type: 'text', text: '請重新點選「立即預約」。' }); currentState.step = 'SURNAME'; currentState.time = gio; userState[userId] = currentState; const displayDate = formatDateDisplay(currentState.date); return client.replyMessage(event.replyToken, { type: 'text', text: `好的，您預約了 ${displayDate} ${gio} (${currentState.pax}位)。\n\n請問怎麼稱呼您？(請輸入姓氏)` }); }
  ...
  if (userState[userId] && userState[userId].step === 'PHONE') { const sdt = normalizePhoneNumber(text); if (!/^\d{7,15}$/.test(sdt)) return client.replyMessage(event.replyToken, { type: 'text', text: '⚠️ 號碼格式錯誤。請輸入正確手機號碼。' }); const currentState = userState[userId]; const serviceName = SERVICES[currentState.service].name; const gio = currentState.time; const displayDate = formatDateDisplay(currentState.date); const hoTen = currentState.surname; const paxDisplay = `${currentState.pax} 位`; let staffDisplay = '隨機'; if (currentState.selectedStaff && currentState.selectedStaff.length > 0) staffDisplay = currentState.selectedStaff.join(', '); const pricePerPerson = SERVICES[currentState.service].price || 0; const totalPrice = (pricePerPerson + (currentState.isOil ? 200 : 0)) * currentState.pax; const confirmMsg = `✅ 預約成功\n\n👤 ${hoTen} (${sdt})\n📅 ${displayDate} ${gio}\n💆 ${serviceName.split('(')[0]}\n👥 ${paxDisplay}\n🛠️ ${staffDisplay}\n${currentState.isOil ? '⭐ 包含油推 (+$200)\n' : ''}💵 總金額: $${totalPrice}`; await client.replyMessage(event.replyToken, { type: 'text', text: confirmMsg }); if (userId !== ID_BA_CHU) client.pushMessage(ID_BA_CHU, { type: 'text', text: `💰 新訂單!\n${confirmMsg}` }); await ghiVaoSheet({ gioDen: gio, ngayDen: currentState.date, dichVu: serviceName, nhanVien: staffDisplay, userId: userId, sdt: sdt, hoTen: hoTen, trangThai: '已預約', pax: currentState.pax, isOil: currentState.isOil }); delete userState[userId]; return; }
  ...
  if (text.includes('預約') || text.toLowerCase().includes('booking') || text.includes('menu') || text.toLowerCase() === 'menu' || text.includes('價目表')) { delete userState[userId]; syncData(); return client.replyMessage(event.replyToken, { type: 'flex', altText: '服務價目表', contents: createMenuFlexMessage() }); }
...
app.listen(port, () => {
    console.log(`Bot v87 (Full Logic + Server DB) running on ${port}`);
});